...
class AddressCorrection:
    ...
    def _wards_correction(self, tokens, prefix_province, province, prefix_district, district,
        ...
        for wards_index in range(max(0, current_district_index - 4), current_district_index):
            ...
            if len(phrase) < 8:
                ...
                distance_th = 30
            wards_candidates = self.correct(
                phrase, correct_wards, distance_threshold=distance_th)
            for wards, wards_distance in wards_candidates:
                ...
                def check_prefix():
                    ...
                    if tokens[wards_index - 1] == 'tt':
                        ...
                        new_wards_index = wards_index - 1
                        return new_wards_index, prefix_wards, distance
                    d = self.string_distance.distance(
                        tokens[wards_index - 1], 'phường')
                    # print(test)
                    if d < 10:
                        prefix_wards = 'phường'
                        new_wards_index = wards_index - 1
                    ...
                    if wards_index < 2:
                        return new_wards_index, prefix_wards, distance
                    d = self.string_distance.distance(
                        ' '.join(tokens[wards_index - 2:wards_index]), 'thị trấn')
                    if d <= 20:
                        prefix_wards = 'thị trấn'
                ...
                wards_normalized = wards + \
                    ',' if prefix_wards is None else '{} {},'.format(
                        prefix_wards, wards)
                address_composition = [wards_normalized,
                                       district_normalized, province_normalized]
                if new_wards_index > 0:
                    underwards_tokens = tokens[:new_wards_index]
                    correct_underwards = self.underwards[(
                    ...
                    for i in range(len(underwards_tokens) - 1, max(-1, len(underwards_tokens) - 5), -1):
                        if not tokens[i].isalpha():
                            break
                        underwards_phrase = ' '.join(
                            underwards_tokens[i:wards_index])
                    ...
                    if corrected_underwards is not None:
                        prefix_address = ' '.join(
                            tokens[:under_wards_index] + [corrected_underwards + ','])
                    else:
                        prefix_address = ' '.join(
                            tokens[:new_wards_index]) + ','
                    address_composition = [
                        prefix_address] + address_composition
        ...
        if result is None and current_distance < 20:
            ...
            result = ' '.join(
                [prefix_address, district_normalized, province_normalized])
    ...
    def _district_correction(self, tokens, prefix_province, province,
        ...
        result = None
        normalized_province = '{} {}'.format(
            prefix_province, province) if prefix_province else province
        result_distance = current_result_distance
                ...
                if district_index > 0:
                    ...
                    def check_prefix():
                        ...
                        if tokens[district_index - 1] == 'tx':
                            ...
                            return new_district_index, prefix_district, distance
                        d = self.string_distance.distance(
                            tokens[district_index - 1], 'thành phố')
                        if d < 30:
                        ...
                        if district_index < 2:
                            return new_district_index, prefix_district, distance
                        d = self.string_distance.distance(
                            ' '.join(tokens[district_index - 2: district_index]), 'thành phố')
                        if d <= 20:
                            prefix_district = 'thành phố'
                            ...
                            return new_district_index, prefix_district, distance
                        d = self.string_distance.distance(
                            ' '.join(tokens[district_index - 2: district_index]), 'thị xã')
                        if d <= 20:
                    ...
                    if new_district_index > 0:
                        ...
                        if new_distance < result_distance:
                            result_distance = new_distance
                            normalized_district = '{} {}'.format(
                                prefix_district, district)
                            result = '{}, {}'.format(
                                normalized_district, normalized_province)
    ...
    def correction(self, address, correct_th=50):
        '''
        ...
        prefix_number = ('số', 'đội', 'xóm', 'khu', 'ngách',
                         'đường', 'tổ', 'ngõ', 'phường')
        for i in range(1, min(5, len(tokens))):
            if not tokens[i].isalpha() and 'p' not in tokens[i]:
                corrected_token = self.correct(
                    tokens[i - 1], prefix_number, nb_candidates=1, distance_threshold=50)[0]
