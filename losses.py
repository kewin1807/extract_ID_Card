...
def balanced_crossentropy_loss(args, negative_ratio=3., scale=5.):
    ...
    negative_count = tf.reduce_min(
        [tf.reduce_sum(negative_mask), positive_count * negative_ratio])
    loss = K.binary_crossentropy(gt, pred)
    positive_loss = loss * positive_mask
    negative_loss = loss * negative_mask
    negative_loss, _ = tf.nn.top_k(tf.reshape(
        negative_loss, (-1,)), tf.cast(negative_count, tf.int32))
...
def dice_loss(args):
    ...
    weights = (weights - tf.reduce_min(weights)) / \
        (tf.reduce_max(weights) - tf.reduce_min(weights)) + 1.
    mask = mask * weights
...
def l1_loss(args, scale=10.):
    ...
    loss = K.switch(mask_sum > 0, tf.reduce_sum(
        tf.abs(pred - gt) * mask) / mask_sum, tf.constant(0.))
    loss = loss * scale
    return loss
...
def db_loss(args):
    ...
    balanced_ce_loss_, dice_loss_weights = balanced_crossentropy_loss([
                                                                      binary, gt, mask])
